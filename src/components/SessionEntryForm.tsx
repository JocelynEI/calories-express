...
import { ACTIVITY_EFFORTS, normallyIncludedInSteps, sessionEnergy, sessionEnergyIssue, validActivityWeight } from '../domain/activity-energy';
...
export function SessionEntryForm({ day, entry, idea, onSave, onCancel, onDirty }: Props) {
  ...
  const [effort, setEffort] = useState<ActivityEffort>(entry?.effort ?? 'easy'), [weight, setWeight] = useState(initialWeight === undefined ? '' : String(initialWeight));
  ...
  const draft: ActivitySession = { id: entry?.id ?? 'preview', day, kind, minutes: readNumber(minutes) ?? NaN, effort, note: note.trim(), includeInGoal, includedInSteps,
    energySource: source, ...(source === 'reported' ? { reportedActiveKcal: readNumber(manualKcal) ?? NaN, deviceName: device.trim() || 'Kcal saisies' } : { weightKg: readNumber(weight) ?? NaN }) };
  ...
  return <View style={form.card}>
    ...
    <View style={form.row}>{(Object.keys(ACTIVITY_LABELS) as ActivityKind[]).map(k => <Choice key={k} label={ACTIVITY_LABELS[k]} selected={kind === k} onPress={() => { setKind(k); setEffort('easy'); setIncludedInSteps(normallyIncludedInSteps(k)); if (k === 'other') setSource('reported'); setError(''); }} />)}</View>
    ...
    {source === 'estimated' ? <>
      <Field label="Poids utilisé pour cette séance (kg)" value={weight} onChange={setWeight} numeric placeholder="Ton poids" maxLength={6} />
